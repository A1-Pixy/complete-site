/*!
 * functions/square-config.mjs — Pixy Dust Seasoning
 *
 ...
 *                           production: "sq0idp-..."
 *   SQUARE_LOCATION_ID    — Square location ID (same for sandbox & production)
...
export default async (req) => {
  ...
  if (!appId || !locationId) {
    console.error("[square-config] ❌ Missing env var(s).",
      "SQUARE_APP_ID set:", !!appId,
      "| SQUARE_LOCATION_ID set:", !!locationId);
    return json(500, { ok: false, error: "Square not configured — set SQUARE_APP_ID and SQUARE_LOCATION_ID in Netlify env" }, corsHeaders());
  }
