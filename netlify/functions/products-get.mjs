...
export default async (req) => {
  ...
  try {
    ...
    console.log("[products-get] Query params:", JSON.stringify(params));
    const rows = await supabaseGet(SUPABASE_URL, SUPABASE_ANON_KEY, "products", params);
    const products = Array.isArray(rows) ? rows : [];
    console.log("[products-get] ✅ Returned", products.length, "products");

    return json(200, { ok: true, products }, corsHeaders());
  } catch (e) {
    console.error("[products-get] ❌ Supabase error:", e.message);
    return json(500, { ok: false, error: "Failed to load products", detail: e.message }, corsHeaders());
  }
};
